 ...
 * 本身不呼叫邊相關 API，只收集表單值後呼叫 onConfirm
 */
export function EdgeSettingsModal({ mode, initialValues, onConfirm, onCancel }: Props) {
  ...
  useEffect(() => {
    TheoryService.findAll().then(setTheories).catch(() => {})
  }, [])
  ...
  const toggleTheory = (id: string) => {
    setValues(prev => ({
      ...prev,
      theoryIds: prev.theoryIds.includes(id)
        ? prev.theoryIds.filter(t => t !== id)
        : [...prev.theoryIds, id],
    }))
  }
  ...
  return (
    ...
    >
      <form
        ...
        style={{ background: '#fff', padding: '24px', borderRadius: '8px', minWidth: '320px', maxWidth: '480px' }}
      >
        ...
        <div style={{ marginBottom: '12px' }}>
          ...
          <textarea
            ...
            style={{ display: 'block', width: '100%', marginTop: '4px', minHeight: '60px' }}
          />
        </div>

        {theories.length > 0 && (
          <div style={{ marginBottom: '16px' }}>
            <label style={{ display: 'block', marginBottom: '6px' }}>理論框架</label>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {theories.map(t => (
                <label key={t.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                  <input
                    type="checkbox"
                    checked={values.theoryIds.includes(t.id)}
                    onChange={() => toggleTheory(t.id)}
                  />
                  {t.name}
                </label>
              ))}
